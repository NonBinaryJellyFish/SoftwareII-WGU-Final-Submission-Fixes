...
public class appointmentsScreen implements Initializable {
    ...
    private ObservableList<String> allContacts =  FXCollections.observableArrayList();

    /**
     * Creates an appointment by first validating values, creating UTC and local timestamps, and obtaining the logged in user's information.
     * @param event
     * @throws Exception
     */
    @FXML
    private void createAppointment(Event event) throws Exception {
        ...
        if((apptStartTime != null || apptEndDate != null || apptStartDate != null || apptEndTime != null) || (apptStartTime == null || apptEndDate == null || apptStartDate != null || apptEndTime != null)) {
            ...
            if(startToSql != null && endToSql != null) {
                isValidStart = isApptStartTimeValid(startToSql.toString(), endToSql.toString());
                isValidEnd = isApptEndTimeValid(startToSql.toString(), endToSql.toString());
            }
            ...
            System.out.println("Start: " + isValidStart + "\n End: " + isValidEnd);

            if((!isValidStart || !isValidEnd) && (apptEndTime != null && apptEndTime != null)) {
            ...
            if(exception.length() == 0 && isValidStart) {
                int custID = Integer.parseInt(strCustID.trim());

                try {

                    PreparedStatement ps = sql.getConn().prepareStatement("INSERT INTO appointments(Title, Description, Location, Type, Start, End, Create_Date, Created_By, Last_Update, Last_Updated_By, Customer_ID, User_ID, Contact_ID) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?);");
                    ...
                    ps.setString(7, String.valueOf(ltdToSql));
                    ps.setString(8, createdBy);
                    ps.setString(9, String.valueOf(ltdToSql));
                    ps.setString(10, createdBy);
                    ps.setInt(11, custID);
                    ps.setInt(12, LoginScreen.getSession().getUserID());
    ...
    /**
     * Validates that the appointment start time is valid and returns a boolean value.
     * @param start
     * @param end
     * @return
     * @throws SQLException
     */
    public boolean isApptStartTimeValid(String start, String end) throws SQLException {
    ...
    /**
     * Validates that the appointment end time is valid and returns a boolean value.
     * @param start
     * @param end
     * @return
     * @throws SQLException
     */
    public boolean isApptEndTimeValid(String start, String end) throws SQLException {
    ...
    /**
     * getContacts() obtains an Observable list of String objects containing the value of Contact Names from the database.
     */
    ...
    /**
     * Exits the current scene, to get back to the main scene.
     * @param event
     * @throws IOException
     */
    @FXML
    ...
    /**
     * Initializes the scene by passing in language values, and other needed functions for the start/end time ranges for 8am-10pm. It also populates the comboboxes.
     * @param url
    ...
    public void initialize(URL url, ResourceBundle resourceBundle){
        ...
        //Lambda function - The reason that this exists is for accessibility when trying to select a customer ID for an appointment.
